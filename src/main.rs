...
fn main() {
    ...
    let subtraccion = left_value - right_value;

    expression = expression.replace(cap_expression, &subtraccion.to_string());
    }
    loop{
    ...
    let divide = left_value / right_value;

    expression = expression.replace(cap_expression, &divide.to_string());
    }
